...
class Solution {
    // 更优解，把非0的数字交换到前面去，但操作次数不是最优，因为不是所有前面有 0 的非0元素都需要交换位置
    public void moveZeroes(int[] nums) {
        int nonZeroPos = 0;
        for(int i = 0; i < nums.length; i++) {
            if(nums[i] != 0) {
...
/**
 * 有一个 follow up 是给出对原数组写入次数最小的 in-place 解，读取次数不限，可以不用保持原始相对位置：
 * 为了让 0 和非 0 数字分开，从 [X, X, X, 0, 0, X, 0, X, X, 0] 变为 
 *                          [X, X, X, X, X, X, 0, 0, 0, 0],
 * 假设 m 个 0 和 n 个 非0，则前 n 个元素中含有的 0 必须被非0 覆盖，而后 m 个 元素中的 非0必须被 0 覆盖
